#!/bin/bash
# This is a camilo-style server script for the Noumena database.
...
setup() {
    ...
    if credentials noumena "$user" "$pass"; then
        ...
        js='use noumena
            ...
            var role = {role: "dbOwner", db: "noumena"};
...
credentials() {
    if [[ -n "$3" ]]; then
        mongo -u "$2" -p "$3" "127.0.0.1:$port/$1" <<< '' &> /dev/null
    else
        mongo "127.0.0.1:$port/$1" <<< '' &> /dev/null
    fi
}
...
execute() {
    ...
    cd "$(base)"
    if [[ -z "$3$4" ]]; then
        mongo "127.0.0.1:$port/$1" <<< "$2" &> /dev/null
        code=$?
    else
        mongo -u "$2" -p "$3" "127.0.0.1:$port/$1" <<< "$4" &> /dev/null
        code=$?
    fi
    cd "$old"
    return $code
    ...
    if running; then
        if ${prefix}kill $flag -s SIGINT "$(process)" &> /dev/null; then
            ...
            if ((retries-- != 0)); then
                ...
                # After 5 attempts in 5 seconds, just go for the jugular.
